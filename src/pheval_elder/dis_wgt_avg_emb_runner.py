...
@dataclass
class DisWgtAvgEmbRunner(PhEvalRunner):
    ...
    def __init__(
        ...
        # Pass only arguments that belong to Parent
        super().__init__(
            input_dir=input_dir,
        ...
        self.weighted_disease_service = self.elder_runner.disease_weighted_service
    ...
    def run(self):
        ...
        if self.elder_runner is not None and self.elder_runner.strategy == "wgt_avg":
            self.results = self.elder_runner.optimized_wgt_avg_analysis(phenotype_sets, self.nr_of_results)
    ...
    def post_process(self):
        # TODO move/create runner_utils.py
...
if __name__ == "__main__":
    ...
    g.global_wgt_avg_disease_embd_collection = runner.weighted_disease_service.disease_new_avg_embeddings_collection
    runner.run()
