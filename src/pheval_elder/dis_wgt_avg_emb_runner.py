"""
Disease Weighted Average Embedding Runner.

This module provides a runner for analyzing phenotype sets using
the weighted average embeddings strategy.
"""
...
@dataclass
class DisWgtAvgEmbRunner(BaseElderRunner):
    """
    Runner for analyzing phenotype sets using the weighted average embeddings strategy.
    
    This runner calculates weighted average embeddings for each phenotype set and
    queries for similar diseases, taking into account phenotype frequencies.
    """

    def run(self) -> None:
        """
        Run the weighted average embeddings analysis on all phenotype sets.
