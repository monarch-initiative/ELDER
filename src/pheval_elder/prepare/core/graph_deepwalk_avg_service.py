...
class GraphDeepwalkAverageEmbeddingService(BaseService):
    ...
    def process_data(self) -> Collection:
        if not self.graph_data_processor.deepwalk_graph_embeddings():
            raise ValueError("Deepwalk Graph Embeddings Dictionary is not initialized.")

        if not self.disease_to_hps:  # Without Frequencies
            raise ValueError("DiseaseToHPs Dictionary is not initialized.")

        # if self.disease_avg_deepwalk_graph_embeddings_collection:
        #     print("Average Deepwalk Graph Embeddings collection already initialized!")
        #     return self.disease_avg_deepwalk_graph_embeddings_collection

        num_diseases = len(self.disease_to_hps)
        batch_size = 100
        current_index = 0
        batch_embeddings = np.zeros((batch_size, 100))
        batch_diseases = np.zeros(batch_size, dtype=object)
        ...
        for disease, hps in tqdm(self.disease_to_hps.items(), total=num_diseases):
            start = time.time()
            average_embedding = self.graph_data_processor.calculate_average_deepwalk_graph_embeddings(
                hps=hps
            )
    ...
    def upsert_batch(self, disease_ids, embeddings):
        ...
        self.disease_avg_deepwalk_graph_embeddings_collection.upsert(ids=filtered_ids, embeddings=embeddings,
                                                                     metadatas=metadatas)
