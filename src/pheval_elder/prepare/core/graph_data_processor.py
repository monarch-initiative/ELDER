...
class GraphDataProcessor:
    ...
    def get_deepwalk_embedings_dict(self):
        return self._deepwalk_graph_embeddings

    def get_line_embeddings_dict(self):
        return self._line_graph_embeddings
    ...
    def calculate_average_line_graph_embeddings(self, hps: list) -> np.ndarray:
        # print(type(self._line_graph_embeddings))
        ...
        return avg_embedding.reshape(1, -1)        ## TODO: typical error new embedding length and need make class instance to pull from for new data processor

    def calculate_average_deepwalk_graph_embeddings(self, hps: list) -> np.ndarray:
        embeddings = [self._deepwalk_graph_embeddings[hp_id] for hp_id in hps if
                      hp_id in self._deepwalk_graph_embeddings]

        if not embeddings:
            logger.info("Embeddings list is empty")
